 ...
 *    description: Dirigirse al siguiente URL ===> https://delilahs-resto.ml/auth0/login
 */

router.get('/auth0/login', passport.authenticate('openidconnect',{prompt: 'login', failureMessage: true}));
...
router.get('/auth0/users', async (req, res,next) => {
  findUser(res, next)
})
...
router.get('/auth0/register', async (req, res,next) => {
  createUser(res, next)
})
...
router.get('/auth0/logged', (req, res) => {
    if(whoIs === ""){
